...
- Le potager est peint directement dans `SCENE_BG` (`bg-garden.jpg`, plus de sprite séparé) ; `SCENE_BED_META` (mesuré sur cette illustration) impose le pas des emplacements : les pots, plantes et jauges sont dessinés à l'échelle du compartiment et posés sur le bord avant de la terre (`bedGroundY`). Le fond impose aussi le ratio de la scène (`layoutScene`). Toute nouvelle illustration de fond doit garder le potager au même endroit/à la même échelle, sinon `SCENE_BED_META` (coordonnées mesurées en px sur l'image 1600×900) doit être remesuré.
- Plante en pleine terre (pas de pot) : rendue via `PLANT_STAGE_SPRITES`/`plantSpriteStage()` — 6 illustrations peintes (seed/germ/young/mature1/mature2/bloom, une seule couleur pour l'instant) plutôt que le dessin procédural pixel. Même convention que le potager : chaque sprite a sa motte de terre peinte en bas de cadre, ancrée sur `bedGroundY()-20`, largeur = `slotPitch()`. Les pots (argile, céramique...) gardent le rendu procédural — la motte peinte n'a pas de sens dans un pot.
...
Pots par matière (argile, céramique, grand céramique, terre cuite, plastique, béton, auto-arrosant), lampe (3 niveaux, allumée/éteinte), goutte-à-goutte (3 niveaux), réservoir, générateur. Prochaine étape sur les plantes : décliner les 6 sprites de croissance par variété (couleur `accent` de chaque type Normie/Hoodie) et gérer les mutations (dorée/double/lumineuse) en overlay plutôt qu'en art séparé.
